...
def main() -> None:
    ...
    if args.action == "test":
        ...
        if test_file.suffix == ".go":
            try:
                rel_dir = test_file.parent.relative_to(root)
                args.test_command = f"cd {shlex.quote(str(rel_dir))} && go test -v"
            except ValueError:
                args.test_command = f"cd {shlex.quote(str(test_file.parent))} && go test -v"
        else:
            try:
